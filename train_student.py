...
from helper.util import save_dict_to_json, reduce_tensor, adjust_learning_rate, extract_weights_except_last
...
def parse_option():
    ...
    #'./save/teachers/models/resnet32x4_vanilla_cifar100_trial_0/resnet32x4_best.pth'

    # distillation
    parser.add_argument('--trial', type=str, default='1', help='trial id')
    parser.add_argument('--kd_T', type=float, default=4, help='temperature for KD distillation')
    parser.add_argument('--distill', type=str, default='atsc', choices=['kd', 'hint', 'attention', 'similarity', 'vid',
                                                                      'crd', 'semckd','srrl', 'simkd', 'atsc'])
    ...
    parser.add_argument('-b', '--beta', type=float, default=0.0, help='weight balance for other losses except ATSC')
    parser.add_argument('-s', '--soft', type=float, default=1.0, help='attention scale of SemCKD')

    # ATSC
    parser.add_argument('-w', type=float, default=1.0, help='weight balance for weight change of teacher for ATSC')
    parser.add_argument('-f', '--factor', type=int, default=2, help='factor size of ATSC and SimKD')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    weight_t_except_last = extract_weights_except_last(model_t, opt)
    ...
    if opt.distill == 'atsc':
        t_last = list(model_t.parameters())[-1]
        optimizer_last = optim.SGD([t_last],
                          lr=opt.learning_rate,
                          momentum=opt.momentum,
                          weight_decay=opt.weight_decay)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if opt.distill == 'atsc':
            adjust_learning_rate(epoch, opt, optimizer_last)

        print("==> training...")

        time1 = time.time()
        if opt.distill == 'atsc':
            train_acc, train_acc_top5, train_loss = train(epoch, train_loader, module_list, criterion_list, optimizer,
                                                          opt, weight_t_except_last, optimizer_last)
        else:
