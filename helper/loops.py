...
import time
import torch
from .util import AverageMeter, accuracy, reduce_tensor, extract_weights_except_last, sum_of_weight_differences
...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt, teacher_weight_except_last = None, optimizer_last = None):
    ...
    batch_time = AverageMeter()
    losses = AverageMeter()
    losses_last = AverageMeter() #only for ATSC
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill == 'kd':
            ...
            logit_s = pred_feat_s
            loss_kd = criterion_kd(trans_feat_s, trans_feat_t)
            cur_teacher_weight_except_last = extract_weights_except_last(module_list[-1], opt, detach = False)
            loss_w = sum_of_weight_differences(teacher_weight_except_last,cur_teacher_weight_except_last) #loss for changes in teacher's weight
        else:
            raise NotImplementedError(opt.distill)

        loss = opt.cls * loss_cls + opt.div * loss_div + opt.beta * loss_kd + opt.w * loss_w
        losses.update(loss.item(), images.size(0))
        ...
        if opt.distill == 'atsc':
            ...
            loss_last = criterion_cls(logit_t, labels)
            losses_last.update(loss_last.item(), images.size(0))
        ...
        if idx % opt.print_freq == 0:
            if opt.distill != 'atsc':
                ...
                print('Epoch: [{0}][{1}/{2}]\t'
                      ...
                      'Time: {batch_time.avg:.3f}\t'
                      'Loss {loss.avg:.4f}\t'
                      'Second Loss {loss_last.avg:.4f}\t'
                      'Acc@1 {top1.avg:.3f}\t'
                      'Acc@5 {top5.avg:.3f}'.format(
                    epoch, idx, n_batch, opt.gpu, loss=losses, loss_last = losses_last, top1=top1, top5=top5,
                    batch_time=batch_time))
                sys.stdout.flush()
    return top1.avg, top5.avg, losses.avg
